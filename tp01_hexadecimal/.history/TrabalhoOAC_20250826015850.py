...
def traduzir_linha(linha_asm):
    ...
    if not partes:
        return "" # Retorna string vazia para linhas em branco

    # Ignora comentários
    if partes[0].startswith('#') or partes[0].startswith('//'):
        return ""
    ...
    if tipo is None:
        return f"# ERRO: Instrucao '{instrucao}' desconhecida."
    ...
    try:
        if tipo == 'R':
            ...
            if '(' in operandos[1]:
                ...
                rs1 = decimal_para_binario(REGISTRADORES[rs1_str.replace(')', '')], 5)
            else:
                rs1 = decimal_para_binario(REGISTRADORES[operandos[1]], 5)
        ...
        return f"# ERRO de sintaxe na linha: '{linha_limpa}'. Detalhe: {e}"
    # ====================================================
    # === LINHAS PARA TRADUZIR BINÁRIO PARA HEXADECIMAL ===
    # ====================================================
    if linha_binaria_completa: # Garante que a linha não está vazia
        valor_inteiro = int(linha_binaria_completa, 2)
        linha_hexadecimal = format(valor_inteiro, '08x')
        return linha_hexadecimal
    # ====================================================

    return "" # Retorna uma string vazia se nenhum tipo foi encontrado

def main():
    if not os.path.exists(PASTA_ENTRADA):
        print(f"ERRO: A pasta de entrada '{PASTA_ENTRADA}' nao foi encontrada.")
    ...
    for arquivo in arquivos_asm_para_traducao:
        if arquivo.endswith('.asm'):
            ...
            linhas_traduzidas_validas = []
            with open(entrada, 'r') as f_entrada:
                for linha_asm in f_entrada:
                    linha_traduzida = traduzir_linha(linha_asm)
                    if linha_traduzida:
                        linhas_traduzidas_validas.append(linha_traduzida)
            
            with open(saida, 'w') as f_saida:
                for linha in linhas_traduzidas_validas:
                    f_saida.write(linha + '\n')

            print("Traducaoo concluida.")
            
            print("-" * 50)
            print(f"Conteudo de '{saida}' gerado no terminal:")
            print("-" * 50)
